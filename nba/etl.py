...
class etl(base):
	...
	def update_player_log(self,game_dates,seasons=None, insert=True, qtr = 0):
        ...
        log = self.get_logs(game_dates,seasons,qtr) #has all 4
        bskt = self.get_first_buckets(game_dates) #playerid,gameid
        rbs = self.get_rebounds(game_dates,qtr) #all 4
        adv = self.get_advanced_box(game_dates,qtr) # gameid,playerid
        shts = self.get_player_shot_spots(game_dates,qtr) #has playerid,gamedate,teamid
	...
	def get_advanced_box(self,game_dates,qtr=0):
        ...
        for gid in tqdm(games.GAME_ID.unique()):
            advbox = BoxScoreAdvancedV3(gid,end_period=qtr).get_data_frames()[0].rename(columns={'gameId':'GAME_ID','personId':'PLAYER_ID'})
